...
describe('streaming response timeouts', () => {
  beforeEach(() => { vi.useFakeTimers(); clearRateLimitState(); });
  afterEach(() => { vi.useRealTimers(); vi.unstubAllGlobals(); });

  it('allows a progressing consumer to exceed the initial timeout', async () => {
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue(new Response('unused')));
    const pending = httpRequest('https://example.com/file', {
      timeoutMs: 30,
      maxRetries: 1,
      consumeResponse: async (_response, resetTimeout) => {
    ...
    expect(result.ok).toBe(true);
    if (result.ok) expect(result.value.data).toBe('streamed');
